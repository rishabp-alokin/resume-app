...
<div class="page">
  ...
  <div class="px-14">
    <section class="resume_content">
      <article>
        <h2 class="pt-6">Education</h2>
        {#each data.education as education}
          <section class="pb-4 pl-4">
            <h3>
              {education.course}
              <span class="time">
                <time datetime="2003-07-04">Marks:{education.marks}</time>
              </span>
            </h3>
            <p>
              <strong>{education.institution} </strong> <b>Pass Out Year: {education.passout_year}</b>
            </p>
          </section>
        {/each}
  
        <h2 class="pt-6">Professional Experience</h2>
        {#each data.work_experience as experience}
          <section class="pl-4">
            <h3>
              {experience.designation}
              <span class="time">
                <time datetime={experience.joining_date} class="start">{experience.joining_date}</time
                >
        ...
        <h2 class="pt-6">Projects Worked</h2>
        {#each data.projects as project}
          <section class="pl-4">
            ...
            <p class="text-justify w-full">{project.description}</p>
          </section>
        {/each}
  
        <h2 class="pt-6 pb-6">Certifications</h2>
        {#each data.certifications as certification}
          <section class="pl-4 pb-3">
            <div class="border-b-2">
              <ul>
                <li class="text-xl font-normal">
                  {certification.certification_name}
                </li>
                <p class="text-justify w-full">{certification.certification_url}</p>
              </ul>
            </div>
          </section>
        {/each}
    ...
    <div class="resume_skills">
      <aside>
        ...
        <section>
          <h2 class="pt-4">Proficent Languages</h2>
          <ul>
        ...
        <section>
          <h2 class="pt-4">Address</h2>
          <p class="capitalize text-wrap max-w-96">
        ...
        <section>
          <h2 class="pt-4">Socials</h2>
          <ul>
            {#each data.social_media as social}
              <li>{social.platform} - {social.url}</li>
            {/each}
          </ul>
        </section>
      </aside>
...
<style lang="css">
	...
	@media screen and (min-width: 320px) {
		...
		.vcard {
			...
			display: none;
			margin: 1em 10px 1em -3px !important;
			position: inherit;
			width: 100%;
		}
		.vcard p {
